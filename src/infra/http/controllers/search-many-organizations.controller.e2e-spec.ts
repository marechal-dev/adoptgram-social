...
describe('Search Many Organizations Controller E2E Test Suit', () => {
  ...
  test('[GET] /api/organizations/text-search', async () => {
    ...
    await Promise.all([
      ...
      organizationFactory.makePrismaOrganization({
        title: 'Lambida de Amor',
        cnpj: Cnpj.create('00.000.000/0001-02'),
      }),
    ]);

    const response = await request(app.getHttpServer())
      .get('/organizations/text-search?query=lamb')
    ...
    expect(response.body).toEqual(
      expect.objectContaining({
        queryResult: expect.arrayContaining([
          expect.objectContaining({
            title: 'Lambeijos de Luz',
          }),
          expect.objectContaining({
